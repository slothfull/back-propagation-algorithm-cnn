...
%    poling layer1?12*12?pooling?*20
%    full-connected layer?100(???)
%    softmax layer?10(softmax)
...
tic
for iter=1:20 % iter is the number of epochs
for n=1:num_sample_to_get_train % each number of character has 800 samples for training
    for m=0:9
        % ????
        ...
        %% compute the loss of training sets
        Error_cost=-output(1,m+1);
        % if (Error_cost<-0.98)
            % break;
        % end
        %% back-propagation parameters refreshing...
...
%% test section
count=0;% count holds the number of accurate identified samples
tic
for n=1:num_sample_to_get_test
    for m=0:9
        % read in the samples for test
